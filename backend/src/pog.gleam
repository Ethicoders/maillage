...
@external(javascript, "./pog_ffi.js", "connect")
pub fn connect(a: Config) -> Connection

/// Shut down a connection pool.
@external(javascript, "./pog_ffi.js", "disconnect")
...
@external(javascript, "./pog_ffi.js", "coerce")
pub fn null() -> Value
...
/// back.
@external(javascript, "./pog_ffi.js", "transaction")
pub fn transaction(
  pool: Connection,
  callback: fn(Connection) -> promise.Promise(Result(t, QueryError)),
) -> promise.Promise(Result(t, TransactionError))
...
@external(javascript, "./pog_ffi.js", "query")
fn run_query(
  a: Connection,
  b: String,
  c: List(Value),
  timeout: Option(Int),
) -> promise.Promise(Result(#(Int, List(Dynamic)), QueryError))
...
pub fn execute(
  query query: Query(t),
  on pool: Connection,
) -> promise.Promise(Result(Returned(t), QueryError)) {
  let parameters = list.reverse(query.parameters)
  use #(count, rows) <- promise.map_try(run_query(
    pool,
    query.sql,
    parameters,
    query.timeout,
