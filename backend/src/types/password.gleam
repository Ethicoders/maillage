...
@external(javascript, "./unicode.js", "characters_to_nfc_binary")
fn characters_to_nfc_binary(str: String) -> String

@external(javascript, "./unicode.js", "characters_to_nfkc_binary")
...
fn normalize(str) {
  let assert Ok(letter_digit) =
    regexp.from_string("[\\p{Ll}\\p{Lu}\\p{Lo}\\p{Nd}\\p{Lm}\\p{Mn}\\p{Mc}]")
  let assert Ok(space) = regexp.from_string("[\\p{Zs}]")
  let assert Ok(symbol) = regexp.from_string("[\\p{Sm}\\p{Sc}\\p{Sk}\\p{So}]")
  let assert Ok(punctuation) =
    regexp.from_string("[\\p{Pc}\\p{Pd}\\p{Ps}\\p{Pe}\\p{Pi}\\p{Pf}\\p{Po}]")
  let assert Ok(other_letter_digit) =
    regexp.from_string("[\\p{Lt}\\p{Nl}\\p{No}\\p{Me}]")

  let graphemes = string.to_graphemes(str)

  use <- bool.guard(
    !list.all(graphemes, fn(g) {
      regexp.check(letter_digit, g)
      || printable_non_space_ascii(g)
      || regexp.check(space, g)
      || regexp.check(symbol, g)
      || regexp.check(punctuation, g)
      || has_compat(g)
      || regexp.check(other_letter_digit, g)
    }),
    Error(Nil),
  )
...
pub fn hash(password: Password) -> promise.Promise(String) {
  argon2.hash(to_string(password))
}
